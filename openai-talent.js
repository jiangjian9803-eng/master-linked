(()=>{
  ...
  root.innerHTML=`<div class="oai-shell">
    ...
    <section class="oai-section"><div class="oai-section-head"><div><h2>人才库与招聘漏斗</h2><p id="oaiResultCount">0 条记录</p></div></div><div class="oai-toolbar"><input id="oaiSearch" placeholder="搜索姓名、职位、技术方向、备注"><select id="oaiTeamFilter"><option value="all">全部团队</option>${teams.map(t=>`<option value="${t.id}">${t.name}</option>`).join('')}</select><select id="oaiStatusFilter"><option value="all">全部状态</option>${Object.entries(statusText).map(([v,n])=>`<option value="${v}">${n}</option>`).join('')}</select><select id="oaiPriorityFilter"><option value="all">全部优先级</option><option value="A">A · 核心</option><option value="B">B · 重点</option><option value="C">C · 储备</option></select><select id="oaiLevelFilter"><option value="all">全部级别判断</option><option value="yes">19+ 潜力</option><option value="review">待判断</option><option value="no">暂不匹配</option></select></div><div id="oaiTable" class="oai-table-wrap"></div></section>
    ...
    <label>当前职位<input id="oaiTitle"></label><label>所在地<input id="oaiLocation"></label>
  ...
  function filtered(){const q=clean($('#oaiSearch').value).toLowerCase(),team=$('#oaiTeamFilter').value,status=$('#oaiStatusFilter').value,priority=$('#oaiPriorityFilter').value,level=$('#oaiLevelFilter').value;return state.people.filter(p=>(team==='all'||p.team===team)&&(status==='all'||p.status===status)&&(priority==='all'||p.priority===priority)&&(level==='all'||p.level19===level)&&[p.name,p.title,p.location,p.focus,p.notes,p.nextAction].join(' ').toLowerCase().includes(q));}
  function render(){
    const rows=filtered();
  ...
  $('#oaiForm').onsubmit=e=>{e.preventDefault();const id=$('#oaiDialog').dataset.id;const data={name:clean($('#oaiName').value),linkedin:clean($('#oaiLinkedin').value).replace(/\/$/,''),title:clean($('#oaiTitle').value),location:clean($('#oaiLocation').value),team:$('#oaiTeam').value,focus:clean($('#oaiFocus').value),priority:$('#oaiPriority').value,level19:$('#oaiLevel19Field').value,status:$('#oaiStatus').value,lastContact:$('#oaiLastContact').value,nextAction:clean($('#oaiNextAction').value),source:clean($('#oaiSource').value),notes:clean($('#oaiNotes').value),updatedAt:new Date().toISOString()};const existing=state.people.find(p=>p.id===id);if(existing)Object.assign(existing,data);else state.people.unshift({id:crypto.randomUUID(),...data});save();$('#oaiDialog').close();render();};
